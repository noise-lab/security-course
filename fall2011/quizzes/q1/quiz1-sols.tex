...
\sols{
\begin{answer}
{\em Vulnerability:} A weakness in the security system that might be
exploited to cause harm.  Example: Software vulnerabilities in the
Windows operating system (and other) that allowed the virus to
spread. {\em Threat:} A set of circumstances that has the potential to
cause loss or harm.  Example: The fact that the centrifuges were
controllable from software.  {\em Attack:} An active attempt to exploit
a vulnerability.  Example: An attempt to destroy the centrifuges using
the software controller.  {\em Exploit:} A tool for mounting a
successful attack.  Example: The Stuxnet worm itself.
\end{answer}
}
...
\sols{
\begin{answer}
Static analysis attempts to analyze source code {\em without executing
  it} in an attempt to reveal vulnerabilities.  Dynamic analysis
actually runs the code in an attempt to uncover vulnerabilities.
Decidability involves answering questions about the behavior about an
algorithm or software program (e.g., will a program terminate).  
Determining whether a piece of source code will terminate is 
undecidable; for this reason, static analysis also cannot uncover all
potential security problems: because static analysis cannot determine
whether a program will terminate, it cannot determine whether a
particular software execution path will lead to a code segment that will
perform some undesirable behavior (e.g., will the program do X before
terminating?).
\end{answer}
}
\newpage
...
\sols{
\begin{answer}
Privilege escalation occurs when an attacker compromises a system and
then uses the initial compromise as a ``stepping stone'' to gain access
to a system with a higher privilege level.  One hypothetical attack on
an automotive system would be compromising the car's wireless network,
and then eavesdropping on communications on that network to dicover
credentials that would allow the attacker to start the car (I am not
sure the paper itself had any privilege escalation attacks).
\end{answer}
}
...
\sols{
\begin{answer}
Permutation scanning takes the ``hit list'' of IP addresses that a host
should scan and randomly permutes the list so that different infected
hosts end up scanning different, random regions of IP address space.
This process accelerates the process of finding vulnerable hosts, since
each newly infected host can scan a relatively non-overlapping set of
vulnerable IP addresses (as opposed to not permuting the list, which
would result in every newly infected host scanning the hosts in the same
order. 
\end{answer}
}
